<template>
  <div class="card-container photoRef" v-if="!$common.isEmpty(props.resourcePathList)">
    <div v-for="(resourcePath, index) in props.resourcePathList" :key="index" class="card-item shadow-box-mini"
      data-aos="zoom-in">
      <div class="card-image container">
        <el-image :style="{
          opacity: hoverIndex === -1 ? 1 : index === hoverIndex ? 1 : 0.2,
        }" crossorigin="anonymous" @mouseenter="handleMouseEnter($event, index)" @mouseleave="handleMouseLeave"
          class="custom-el-image" lazy :preview-src-list="[resourcePath.cover]" :src="resourcePath.cover" fit="cover">
          <template v-slot:error>
            <div class="image-slot">
              <div class="error-aside-image">
                <div class="error-text">图片遗失在外太空咯~┮﹏┭</div>
              </div>
            </div>
          </template>
        </el-image>
